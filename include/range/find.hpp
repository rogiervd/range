    ...
    */
    ...
    public:
        ...
        auto operator()(Range && range, Direction const & direction)
            -> decltype(rime::call_if(
                range::is_homogeneous<Range>(),
                std::declval<when_homogeneous_>(),
                std::declval<when_heterogeneous_>(), std::declval<finder &>(),
                std::forward<Range>(range), direction))
        {
            return rime::call_if(
                range::is_homogeneous<Range>(), when_homogeneous_(),
                when_heterogeneous_(), *this, std::forward<Range>(range),
                direction);
        }
    };
    ...
    */
    ...
    struct when_homogeneous
    {
        ...
        */
        ...
        {
            typedef decltype(std::declval<Predicate>()(range::first(
                std::declval<Range>(),
        ...
        template <class Range, class Direction> typename boost::lazy_enable_if<
            always_empty<Range, Direction>, result_of<EmptyActor(Range)>>::type
            operator()(
                finder_ & f, Range range, Direction const & direction) const
        {
            // "range" is used as the local variable.
    ...
    struct when_heterogeneous
    {
        ...
        struct when_empty
        {
            template <class Range, class Direction> auto operator()(
                finder_ & f, Range && range, Direction const &) const
...
namespace callable {
    ...
    private:
        struct dispatch
        {
            // All arguments.
            template <
                class Range, class Direction, class Predicate,
                class NonEmptyActor, class EmptyActor>
            auto operator()(
            ...
            // No empty_actor.
            template <
                class Range, class Direction, class Predicate, class Actor>
            auto operator()(
                Range && range, Direction const & direction,
    ...
    public:
        // With direction.
        template <
            class Range, class Direction, class... Rest,
            class Enable =
        ...
        template <
            class Range, class Argument, class... Rest,
            class Enable =
