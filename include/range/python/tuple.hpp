    ...
    */
    template <class Tuple> struct tuple_caster {
        ...
        static_assert (std::is_same <Tuple, typename std::decay <Tuple>::type
            >::value, "tuple_caster requires an unqualified type.");
        ...
        static nanobind::handle from_cpp (Tuple const & tuple,
            nanobind::rv_policy, nanobind::detail::cleanup_list *)
        {
            ...
            return detail::construct_tuple (range::view (tuple),
                typename meta::count_c <size>::type()).release();
        }
    };
