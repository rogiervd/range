...
public:
    static_assert(
        range::is_view<Underlying, Direction>::value,
...
public:
    template <class CVUnderlying> take_range(
        CVUnderlying && underlying, Limit const & limit,
        Direction const & direction)
    : underlying_(std::forward<CVUnderlying>(underlying)), limit_(limit),
...
private:
    ...
    template <
        ...
        class Enable =
            typename std::enable_if<!rime::is_constant<Limit2>::value>::type>
    ...
    namespace implementation {
        ...
        private:
            struct when_not_take_range
            {
                ...
                template <
                    class Range2, class Limit, class Direction,
                ...
                Range2 operator()(
                    Range2 && range, Limit const & limit,
                ...
                template <
                    ...
                    class Size = typename result_of<
                        callable::size(Range2 const &, Direction)>::type,
                    class Difference =
                        decltype(std::declval<Size>() - take_detail::min()(std::declval<Limit>(), std::declval<Size>())),
            ...
            {
                template <
                    class Underlying, class OriginalLimit, class Limit,
                    class Direction>
                auto operator()(
            ...
            struct is_take_range_with_same_direction
            {
                ...
                template <
                    class Underlying, class OriginalLimit, class Direction>
                auto operator()(
                    take_range<Underlying, OriginalLimit, Direction> const &
                        range,
        ...
        public:
            ...
            template <
                class Range, class Limit, class Direction,
...
namespace take_operation {
    ...
    inline auto implement_drop(
        take_range_tag const &, TakeRange && r, Increment const & increment,
        Direction const & direction)
        -> decltype(range::take(
            range::drop(
                range::helper::get_underlying<TakeRange>(
    ...
    {
        rime::assert_(direction == r.direction());
        rime::assert_(!rime::less_sign_safe(increment, rime::size_t<0>()));
        rime::assert_(!rime::less_sign_safe(r.limit(), increment));
        typedef typename std::decay<TakeRange>::type::limit_type limit_type;
        return range::take(
            range::drop(
                range::helper::get_underlying<TakeRange>(r), increment,
                direction),
    ...
    */
    template <
        class TakeRange, class Direction,
        class Limit = typename std::decay<TakeRange>::type::limit_type,
        class NewLimit = decltype(rime::cast_value<Limit>(
            Limit() - callable::implementation::one_type())),
        ...
        class Result =
            chopped<typename FirstAndUnderlyingRest::first_type, NewTakeRange>>
