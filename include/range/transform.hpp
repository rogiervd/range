...
private:
    ...
    template <class Direction> auto chop_in_place(Direction const & direction)
        RETURNS(
            function_.content()(range::chop_in_place(underlying_, direction)));
};
...
namespace transform_operation {
    ...
    template <class View, class Direction> inline auto implement_first(
        transform_view_tag const &, View && view, Direction const & direction)
