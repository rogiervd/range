...
namespace tuple_detail {
    ...
    */
    template <class Type> struct contained_type
    {
        typedef Type type;
    };
    ...
    */
    ...
    public:
        ...
        classes.
        */
        contain & get_contain(contain_index<Index>) { return *this; }
    ...
    */
    ...
    public:
        ...
        {
            static_assert(
                Index < tuple_size<Tuple>::value, "Tuple index out of range.");

            typedef decltype(std::declval<Tuple>()
                                 .elements()
                                 .get_contained_type(
                                     contain_index<Index>())) contain_type;
        ...
        typename result<Tuple>::type operator()(Tuple && tuple) const
        {
            ...
            return implementation(
                tuple.elements().get_contain(contain_index<Index>()).element_);
        }
    };
    ...
    */
    ...
    private:
        ...
        typedef typename utility::storage::store<First>::type first_stored_type;
        ...
        */
        template <
            class Range,
            bool NeverEmpty = never_empty<Range, direction::front>::value>
        ...
        : boost::mpl::and_<
              std::is_convertible<
                  typename result_of<callable::first(
        ...
        */
        template <
            class Range,
            bool AlwaysEmpty = always_empty<Range, direction::front>::value>
        ...
        */
        template <
            class Range,
            bool KnownEmpty = always_empty<Range, direction::front>::value>
        ...
        static constexpr bool is_move_assignable = rest_type::is_move_assignable
    ...
    public:
        ...
        */
        ...
        explicit elements(
            from_elements, FirstArgument && first_argument,
            OtherArguments &&... other_arguments)
        : contain_type(std::forward<FirstArgument>(first_argument)),
          rest_type(
              from_elements(), std::forward<OtherArguments>(other_arguments)...)
        {}
        ...
        */
        ...
        \throw size_mismatch if the range is not empty.
        */
        template <class Range> static typename boost::enable_if<
            boost::mpl::and_<
        ...
        template <class Range> elements(from_range, Range && range)
        : contain_type(range::first(range, front)),
          rest_type(
              from_range(),
              rest_type::maybe_chop(
        ...
        \throw size_mismatch if the range is not empty.
        */
        template <class Range> static typename boost::lazy_disable_if<
            boost::mpl::and_<
                has<callable::first(Range const &, direction::front)>,
                has<callable::drop(Range, direction::front)>>,
        ...
        elements(elements const & that) : contain_type(that), rest_type(that) {}
...
template <class... Types> class tuple
{
    ...
    */
    typedef typename std::conditional<
        elements_type::is_copy_assignable, tuple, dummy_type>::type
        tuple_if_copy_assignable;
    ...
    */
    typedef typename std::conditional<
        elements_type::is_move_assignable, tuple, dummy_type>::type
        tuple_if_move_assignable;
...
public:
    ...
    */
    template <
        class Types2 = meta::vector<Types...>,
        class Enable = typename boost::enable_if<
    ...
    */
    template <
        class... Arguments,
        class Enable = typename boost::enable_if<utility::are_constructible<
            stored_types, meta::vector<Arguments...>>>::type>
    explicit tuple(Arguments &&... arguments)
    : elements_(
          tuple_detail::from_elements(), std::forward<Arguments>(arguments)...)
    {}
    ...
    */
    ...
    : elements_(
          tuple_detail::from_range(),
          elements_type::maybe_chop(
    ...
    */
    template <
        ...
        class Enable = typename boost::enable_if<boost::mpl::and_<
            not_this_tuple<Range>, is_range<Range>,
            range_is_constructible_but_not_convertible<Range>>>::type>
    ...
    */
    template <
        ...
        class Enable = typename boost::enable_if<boost::mpl::and_<
            not_this_tuple<Range>, is_range<Range>,
            range_is_assignable<Range>>>::type>
...
namespace make_tuple_from_detail {
    ...
    {
        typedef typename result_of<callable::view_once(
            Range, direction::front)>::type view_type;
        static_assert(
            !is_homogeneous<view_type, direction::front>::value,
            "The range passed in is homogeneous and potentially infinite. "
            "Unable to convert it into a tuple.");
        typedef typename tuple_from_types<
            Transform, typename element_types<view_type>::type>::type type;
    };
...
inline auto operator==(tuple<Types1...> const & t1, tuple<Types2...> const & t2)
    RETURNS(equal(t1, t2));

/** \brief
Compare two tuples for inequality.
*/
template <class... Left, class... Right> inline auto operator!=(
...
namespace callable {
    ...
    {
        template <class... Types>
        inline tuple<typename std::decay<Types>::type...> operator()(
    ...
    struct make_tuple_from
    {
        template <
            class Range,
            class Result = typename make_tuple_from_detail::tuple_from<
                std::decay, Range>::type>
...
namespace tuple_detail {
    ...
    public:
        ...
        tuple_view(tuple_view<ThatBegin, ThatEnd, TupleReference> const & that)
        : tuple_(that.pointer())
        {
    ...
    private:
        ...
        struct not_front;
        typedef typename std::conditional<
            is_empty, not_front, direction::front>::type front_if_not_empty;

        struct not_back;
        typedef
            typename std::conditional<is_empty, not_back, direction::back>::type
                back_if_not_empty;
        ...
        typename Extract::template result<TupleReference>::type first(
            direction::front const &) const
        {
        ...
        template <
            class Index,
            class Enable =
                typename std::enable_if<(Index::value < view_size)>::type,
            class Extract =
                extract<(tuple_size - begin_position - Index::value - 1)>>
        typename Extract::template result<TupleReference>::type at_constant(
            Index const & index, direction::front) const
        {
            return Extract()(tuple());
        }
        ...
        template <
            ...
            class Enable =
                typename std::enable_if<(Index::value < view_size)>::type,
            class Extract = extract<(tuple_size - end_position + Index::value)>>
        typename Extract::template result<TupleReference>::type at_constant(
            Index const & index, direction::back) const
        {
            return Extract()(tuple());
        }
        ...
        template <
            class Increment,
            class Enable =
                typename std::enable_if<(Increment::value <= view_size)>::type,
            class Result = tuple_detail::tuple_view<
                Begin + Increment::value, End, TupleReference>>
        Result drop_constant(
            Increment const & increment, direction::front) const
        {
            return Result(tuple());
        }
        ...
        Result drop_constant(Increment const & increment, direction::back) const
        {
            return Result(tuple());
        }
        ...
        elements.
        */
        template <class Function, std::size_t... Indices> void apply_for_each(
        ...
        void for_each(direction::front, Function && function) const
        {
            apply_for_each(
...
template <std::size_t Begin, std::size_t End, class TupleReference>
struct tag_of_qualified<tuple_detail::tuple_view<Begin, End, TupleReference>>
{
...
namespace tuple_operation {
    ...
    inline Result implement_make_view(
        tuple_tag, bool once, tuple<Types...> const & tuple,
    ...
    inline Result implement_make_view(
        tuple_tag, bool once, tuple<Types...> & tuple, helper::front_or_back,
    ...
    template <
        ...
        class Result = typename tuple_detail::tuple_view<
            0, sizeof...(Types), tuple<Types...> &&>>
    inline Result implement_make_view(
        tuple_tag, rime::true_type once, tuple<Types...> && tuple,
        helper::front_or_back, helper::front_or_back = helper::front_or_back())
    {
