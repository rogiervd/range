...
namespace callable {
    ...
    public:
        template <
            class FirstRange, class... RestRanges,
            ...
            class DefaultDirection =
                typename decayed_result_of<default_direction(FirstRange)>::type,
            class Result = zip_range<
                DefaultDirection,
            ...
            FirstRange && first_range, RestRanges &&... rest_ranges) const
        {
            ...
            return Result(
                direction,
                range::view(std::forward<FirstRange>(first_range), direction),
                range::view(
                    std::forward<RestRanges>(rest_ranges), direction)...);
        }
    };
...
namespace zip_from_detail {
    ...
    template <
        class RangeTuple, class Direction,
        class Types = typename meta::as_vector<element_types<RangeTuple>>::type>
...
template <
    class RangeTuple, class Direction,
    class Enable1 = typename std::enable_if<is_range<RangeTuple>::value>::type,
    class Enable2 =
        typename std::enable_if<is_direction<Direction>::value>::type,
    class Result =
        typename zip_from_detail::zip_range_from<RangeTuple, Direction>::type>
inline Result zip_from(RangeTuple && range_tuple, Direction const & direction)
{
    return Result(
        direction,
        transform(
            std::forward<RangeTuple>(range_tuple), lazy::view(direction),
            front));
}
...
inline auto zip_from(RangeTuple && range_tuple) RETURNS(zip_from(
...
public:
    ...
    static_assert(
        meta::all_of_c<std::is_same<
            Ranges, typename std::decay<Ranges>::type>::value...>::value,
        "All range types must be unqualified.");

    static_assert(
        meta::all_of_c<is_range<Ranges>::value...>::value,
...
public:
    template <
        class... Arguments,
        class Enable = typename utility::disable_if_variadic_same_or_derived<
...
namespace zip_operation {
    ...
    namespace zip_detail {
        ...
        */
        ...
        /* implemented_all. */
        template <
            class Callable, class ZipRange, class Direction,
            class Ranges = typename std::decay<ZipRange>::type::range_types>
        ...
        // separately.
        template <
            class ZipRange, class Increment, class Direction,
            class Ranges = typename std::decay<ZipRange>::type::range_types>
        struct drop_implemented_all;

        // Lvalue.
        template <
            class ZipRange, class Increment, class Direction, class... Ranges>
        struct drop_implemented_all<
    ...
    // first.
    template <
        class Direction, class ZipRange,
        class Enable = typename boost::enable_if<
            zip_detail::first_implemented_all<ZipRange &&, Direction>>::type>
    inline auto implement_first(
        zip_range_tag<Direction> const &, ZipRange && r,
    ...
    // drop.
    template <
        class Direction, class ZipRange, class Increment,
        class Enable =
            typename boost::enable_if<zip_detail::drop_implemented_all<
    ...
    template <
        ...
        class Result = chopped<
            decltype(range::callable::first_direct()(
                std::declval<ZipRange const &>(), std::declval<Direction>())),
            decltype(range::callable::drop_direct()(
                std::declval<ZipRange const &>(), std::declval<Direction>()))>>
    inline Result implement_chop(
        zip_range_tag<Direction> const &, ZipRange const & r,
        Direction const & direction)
    {
        ...
        Direction const & direction)
    {
        ...
        return Result(std::move(first), NewRest(direction, std::move(rest)));
    }
