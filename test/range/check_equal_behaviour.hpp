...
typename boost::enable_if<rime::is_constant<Reference>>::type check_equal_value(
    Value const & value, Reference const & reference)
{
    static_assert(
        rime::is_rime_constant<Value>::value, "Value must be a Rime constant");
    static_assert(
        rime::same_constant<Value, Reference>::value,
...
{
    static_assert(
        std::is_same<Value, Reference>::value,
...
void check_equal_value(
    std::pair<Value1, Value2> const & value,
    std::pair<Reference1, Reference2> const & reference)
{
    static_assert(
        std::is_same<Value1, Reference1>::value,
        "value and reference must be of the same type");
    static_assert(
        std::is_same<Value2, Reference2>::value,
        "value and reference must be of the same type");
...
void check_empty(Range const & range, Reference const & reference)
{
    ...
    static_assert(
        range::has<range::callable::empty(Range, direction::front)>::value, "");
    static_assert(
        range::has<range::callable::empty(Range, direction::back)>::value, "");
    check_equal_value(range::empty(range), range::empty(reference));
    check_equal_value(
        range::empty(range, range::front),
        range::empty(reference, range::front));
    check_equal_value(
        range::empty(range, range::back), range::empty(range, range::front));
}

// Check size.
template <class HasSize, class Range, class Reference>
typename boost::enable_if<HasSize>::type check_size(
    Range const & range, Reference const & reference)
{
    static_assert(
        range::has<range::callable::size(Range, direction::front)>::value, "");
    static_assert(
        range::has<range::callable::size(Range, direction::back)>::value, "");
    check_equal_value(
        range::size(range, range::front), range::size(reference, range::front));
    check_equal_value(
        range::size(range, range::back), range::size(range, range::front));
}
template <class HasSize, class Range, class Reference>
typename boost::disable_if<HasSize>::type check_size(
    Range const & range, Reference const & reference, void * = 0)
{
    static_assert(
        !range::has<range::callable::size(Range, direction::front)>::value, "");
}
...
void check_equal_behaviour_recursive(
    bool use_chop, Range const & range, Reference const & reference)
{
    ...
    rime::call_if(
        ...
        nothing, use_chop, range, reference);

    rime::call_if(
        rime::and_(!range::empty(range), !range::empty(reference), HasBack()),
        check_equal_behaviour_recursive_from<
...
struct check_equal_behaviour_recursive_from
{
    template <class Range, class Reference> void operator()(
        bool use_chop, Range const & range, Reference const & reference) const
    {
        ...
        if (!use_chop) {
            check_equal_behaviour_recursive<HasSize, HasBack>(
                !use_chop, ::range::drop(range, d),
                ::range::drop(reference, d));
        } else {
            check_equal_behaviour_recursive<HasSize, HasBack>(
                !use_chop, first_and_rest.forward_rest(),
                ::range::drop(reference, d));
        }
    }
};
...
template <class HasSize, class HasBack, class Zero> struct check_drop_n
{
    Zero zero;
    check_drop_n() : zero() {}
    ...
    */
    ...
    private:
        ...
        typename boost::disable_if<range::always_empty<Range, Direction>>::type
            check_first_equal(
        ...
        template <class Range, class Direction>
        typename boost::enable_if<range::always_empty<Range, Direction>>::type
            check_first_equal(
                Range const & range1, Range const & range2,
                Direction const & direction) const
        {}

        template <class Range1, class Direction, class Range2> void check_equal(
            Range1 const & range1, Range2 const & range2,
            Direction const & direction) const;
        // Not defined.
        ...
        */
        template <class Range, class Direction> void check_equal(
            Range const & range1, Range const & range2,
            Direction const & direction) const
        {
            check_equal_value(
        ...
        */
        ...
        {
            template <
                class Range1, class Range2, class Increment, class Direction>
            void operator()(
                Range1 const & current, Range2 const & reference, Increment gap,
                Direction const & direction) const
            {
                check_equal_value(
                    range::at(current, gap, direction),
    ...
    public:
        ...
        */
            ...
            Direction const & direction) const
        {
            ...
            rime::call_if(
                !range::empty(new_current), check_at(), nothing, current,
                reference, gap, direction);

            // Recursively check combinations of drop with increment.
            // Recurse with drop (n + 1).
            rime::call_if(
                !range::empty(new_current), *this, nothing, current,
                new_reference, new_gap, direction);

            // Recurse starting at this point.
            rime::call_if(
                !range::empty(new_current), *this, nothing, new_current,
                new_reference, zero, range::front);
            rime::call_if(
                rime::and_(!range::empty(new_current), HasBack()), *this,
                nothing, new_current, new_reference, zero, range::back);
        }
    };

    template <class Range, class Direction>
    void operator()(Range const & range, Direction const & direction) const
    {
        rime::call_if(
            range::empty(range, Direction()), nothing, step(), range, range,
            zero, direction);
    }
};
...
template <
    class HasSize, class HasBack, class HasDropConstantN, class HasDropRuntimeN,
...
void check_equal_behaviour(Range const & range, Reference const & reference)
{
    ...
    rime::call_if(
        ...
        range, range::front);
    rime::call_if(
        rime::and_(HasDropRuntimeN(), HasBack()),
        detail::check_drop_n<HasSize, HasBack, std::size_t>(), detail::nothing,
        range, range::back);
    ...
    rime::call_if(
        ...
        detail::nothing, range, range::front);
    rime::call_if(
        rime::and_(HasDropConstantN(), HasBack()),
        detail::check_drop_n<HasSize, HasBack, rime::size_t<0>>(),
        detail::nothing, range, range::back);
}
