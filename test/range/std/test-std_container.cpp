...
BOOST_AUTO_TEST_CASE(test_std_vector_adaptor)
{
...

    {
        ...
        BOOST_MPL_ASSERT((std::is_same<decltype(view), decltype(other_view)>) );
        ...
        BOOST_MPL_ASSERT_NOT((has<callable::chop_in_place(decltype(view))>) );
    }
    ...
    utility::tracked_registry r;
    {
        ...
        BOOST_MPL_ASSERT((std::is_same<decltype(first(v)), tracked<int> &&>) );
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(first(v, back)), tracked<int> &&>) );
...
template <std::size_t Number, class Type> void compare_array(
    std::vector<Type> const & v, std::array<Type, Number> const & a)
{
...
void compare_sequence_containers(std::vector<Type> const & v)
{
    ...
    check_equal_behaviour<false_type, false_type, false_type, false_type>(
        fl, v);
    check_equal_behaviour<false_type, true_type, false_type, false_type>(l, v);
}
