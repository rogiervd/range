...
BOOST_AUTO_TEST_CASE(example_decrement_unsigned)
{
    ...
    }
    {
        ...
        {
            static_assert(
                std::is_same<decltype(i), std::size_t &&>::value,
...
template <
    class True, class Zero, class One, class Two, class Three, class Four,
    class Five, class Six>
void check_finite(
    True true_, Zero zero, One one, Two two, Three three, Four four, Five five,
    Six six)
{
    {
        auto c = count(zero);
...
template <
    class True, class MinusTwo, class MinusOne, class Zero, class One,
    class Two>
void check_finite_negative(
    True true_, MinusTwo minus_two, MinusOne minus_one, Zero zero, One one,
    Two two)
{
    {
    ...
    MinusTwo minus_two, MinusOne minus_one, Zero zero, One one, Two two)
{
    ...
    static_assert(!range::has<range::callable::size(decltype(c))>::value, "");
    static_assert(
        !range::has<range::callable::size(
            direction::front, decltype(c))>::value,
        "");
    static_assert(
        !range::has<range::callable::first(
            direction::back, decltype(c))>::value,
        "");
...
BOOST_AUTO_TEST_CASE(heterogeneous)
{
    // size_t.
    check_finite(
        rime::true_, rime::size_t<0>(), rime::size_t<1>(), rime::size_t<2>(),
        rime::size_t<3>(), rime::size_t<4>(), rime::size_t<5>(),
        rime::size_t<6>());

    // int.
    check_finite(
        rime::true_, rime::int_<0>(), rime::int_<1>(), rime::int_<2>(),
        rime::int_<3>(), rime::int_<4>(), rime::int_<5>(), rime::int_<6>());
    check_finite_negative(
        rime::true_, rime::int_<-2>(), rime::int_<-1>(), rime::int_<0>(),
        rime::int_<1>(), rime::int_<2>());

    // short.
    check_finite(
        rime::true_, rime::constant<short, 0>(), rime::constant<short, 1>(),
        rime::constant<short, 2>(), rime::constant<short, 3>(),
        rime::constant<short, 4>(), rime::constant<short, 5>(),
        rime::constant<short, 6>());
    check_finite_negative(
        rime::true_, rime::constant<short, -2>(), rime::constant<short, -1>(),
    ...
    check_infinite(
        rime::int_<0>(), rime::int_<1>(), rime::int_<2>(), rime::int_<3>(),
        rime::int_<4>());
    check_infinite_negative(
        rime::int_<-2>(), rime::int_<-1>(), rime::int_<0>(), rime::int_<1>(),
        rime::int_<2>());
...
BOOST_AUTO_TEST_CASE(homogeneous)
{
    // size_t.
    check_finite(
        true, std::size_t(0), std::size_t(1), std::size_t(2), std::size_t(3),
    ...
    // short.
    check_finite(
        true, short(0), short(1), short(2), short(3), short(4), short(5),
        short(6));
    check_finite_negative(
        true, short(-2), short(-1), short(0), short(1), short(2));
    ...
    check_infinite_negative(short(-2), short(-1), short(0), short(1), short(2));
