...
#define CHECK_range_equal(r1, r2, value, approximately_value)            \
    ...
    RIME_CHECK_EQUAL(range::equal(r2, r1, range::back), value);          \
                                                                         \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r1, r2, approximately_equal), approximately_value); \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r2, r1, approximately_equal), approximately_value); \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r1, r2, range::front, approximately_equal),         \
        approximately_value);                                            \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r2, r1, range::front, approximately_equal),         \
        approximately_value);                                            \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r1, r2, range::back, approximately_equal),          \
        approximately_value);                                            \
    RIME_CHECK_EQUAL(                                                    \
        range::equal(r2, r1, range::back, approximately_equal),          \
        approximately_value)
...
{
    CHECK_range_equal(
        std::make_tuple(), std::make_tuple(), rime::true_, rime::true_);

    // Different lengths: known at compile-time.
    CHECK_range_equal(
        std::make_tuple(), std::make_tuple(1), rime::false_, rime::false_);
    CHECK_range_equal(
        std::make_tuple(), std::make_tuple(1, 2), rime::false_, rime::false_);

    CHECK_range_equal(
        std::make_tuple(1, 2), std::make_tuple(1), rime::false_, rime::false_);
    CHECK_range_equal(
    ...
    CHECK_range_equal(
        std::make_tuple(5), std::make_tuple(), rime::false_, rime::false_);
    CHECK_range_equal(std::make_tuple(3), std::make_tuple(4), false, true);
    CHECK_range_equal(std::make_tuple(3), std::make_tuple(7), false, false);

    CHECK_range_equal(std::make_tuple(1, 2), std::make_tuple(1, 2), true, true);
    CHECK_range_equal(
        std::make_tuple(1, 2), std::make_tuple(1, 3), false, true);
    CHECK_range_equal(
        std::make_tuple(1, 2), std::make_tuple(1, 5), false, false);
    CHECK_range_equal(
        std::make_tuple(5, 2), std::make_tuple(1, 2), false, false);
    CHECK_range_equal(
        std::make_tuple(5, 2), std::make_tuple(4, 2), false, true);
...
BOOST_AUTO_TEST_CASE(test_range_equal_constants)
{
    ...
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(), std::make_tuple()), rime::true_);

    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(one), std::make_tuple(one)), rime::true_);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(one), std::make_tuple(one, three)),
        rime::false_);

    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(one, three), std::make_tuple(one, three)),
        rime::true_);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(one, three), std::make_tuple(one, four)),
        rime::false_);

    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(one), std::make_tuple(one, 3)),
        rime::false_);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(1), std::make_tuple(one)), true);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(2), std::make_tuple(one)), false);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(four, 1), std::make_tuple(four, one)),
        true);
    RIME_CHECK_EQUAL(
        range::equal(std::make_tuple(four, 2), std::make_tuple(four, one)),
        false);
}
