...
public:
    ...
    template <class Left> auto operator()(Left const & left) const
        RETURNS(left < right);
};
...
{
    template <class T> end_marker operator()(T const &) const
    {
        return end_marker();
    }
};
...
struct return_false
{
    template <class T> rime::false_type operator()(T const &) const
    {
        return rime::false_;
    }
};
...
BOOST_AUTO_TEST_CASE(test_range_find_homogeneous)
{
...

    {
        ...
        BOOST_MPL_ASSERT((std::is_same<decltype(view(v)), decltype(result)>) );
    ...
    {
        auto result = find(v, back, less_than_4, size, range::callable::size());
        RIME_CHECK_EQUAL(result, std::size_t(0));
    }
    // With actors.
    {
    ...
    {
        auto result = find(v, back, less_than_4, range::callable::size(), size);
        RIME_CHECK_EQUAL(result, std::size_t(2));
    }
