    ...
    // This shows how the macro works.
    {
        ...
        */
        ...
        if (bool done = false) {
            ...
            // This for-loop defines a variable "sequence" which holds a
            // reference (an lvalue or rvalue reference, as appropriate) to the
            // sequence. This makes sure the sequence does not prematurely
            // disappear from memory.
            for (auto && sequence = is; !done; done = true)
                // The boolean "seen" seems silly, but it serves two purposes:
                //  1.  The inner for loop is to give the variable "element"
                //  scope.
                //  2.  If there is a "break" statement, the variable will have
                //  an
                //      unexpected value, which signals that the loop can be
                //      aborted.
                if (bool seen = false) {
                } else
                    for (auto v = range::view(
                             static_cast<decltype(sequence)>(sequence));
                         !range::empty(v); seen = !seen)
                        if (seen)
                            // This can only happen if the inner loop has hit a
                            // "break".
                            break;
                        else
                            // Declare a variable of the type that
                            // range::chop_in_place (v) returns. "auto &&" binds
                            // correctly to all kinds of types: lvalue or
                            // rvalue, qualified or not.
                            for (auto && element = range::chop_in_place(v);
                                 !seen;
                                 // ((void) element) is a statement that returns
                                 // "void". It prevents warnings about "element"
                                 // being unused if it is unused. This happens
                                 // if you want to execute something n times
                                 // with: RANGE_FOR_EACH (_, range::count (n)) {
                                 // ... }
...
BOOST_AUTO_TEST_CASE(test_macro)
{
    ...
    // Const-ness
    {
        RANGE_FOR_EACH(i, is) { BOOST_CHECK(!is_const(i)); }
        RANGE_FOR_EACH(i, longer_and_longer()) { BOOST_CHECK(is_const(i)); }
        RANGE_FOR_EACH(i, longer_and_longer_reference())
        {
    ...
    // Embed RANGE_FOR_EACH in an if-else.
    {
        ...
        if (false)
            RANGE_FOR_EACH(dummy, is) { BOOST_CHECK(dummy != dummy); }
        else {
