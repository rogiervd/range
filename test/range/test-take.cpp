...
BOOST_AUTO_TEST_CASE(test_range_take_vector_runtime)
{
    ...
    }
    {
        ...
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(take_back), decltype(take_back_next)>) );
        ...
        take(v, std::size_t(3), front), v);
    {
        ...
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(take_back), decltype(take_back_next_2)>) );
...
BOOST_AUTO_TEST_CASE(test_range_take_list_runtime)
{
...

    {
        ...
        auto first = take(l, 3);
        BOOST_MPL_ASSERT_NOT((std::is_same<decltype(view), decltype(first)>) );
        static_assert(
        ...
        BOOST_MPL_ASSERT((std::is_same<decltype(first), decltype(first_2)>) );
...
BOOST_AUTO_TEST_CASE(test_range_take_input_range)
{
    ...
    {
        auto t =
            range::take(range::make_function_range(produce_consecutive), 3);
    ...
    // Test that chop_in_place with non-homogeneous limit does not exist.
    {
        ...
        BOOST_MPL_ASSERT_NOT(
            (range::has<range::callable::chop_in_place(decltype(t) &)>) );
    }
}
