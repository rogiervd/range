...
BOOST_AUTO_TEST_CASE(test_range_make_tuple)
{
    ...
    }
    {
        ...
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(t), range::tuple<tracked<int>>>) );
    ...
    // Noncopyable: must move.
    {
        ...
        auto t = make_tuple(std::move(i));
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(t), range::tuple<std::unique_ptr<int>>>) );
        BOOST_CHECK_EQUAL(*first(t), 11);
    }
    // More elements.
    {
        auto t = make_tuple(6, 'a');
        BOOST_MPL_ASSERT((std::is_same<decltype(t), range::tuple<int, char>>) );
        BOOST_CHECK_EQUAL(first(t), 6);
    ...
    }
    {
        ...
        float f = 9.5f;
        double d = 13.75;
        auto t = make_tuple(std::move(i), std::move(f), std::move(d));
        BOOST_MPL_ASSERT(
            (std::is_same<decltype(t), range::tuple<int, float, double>>) );
...
BOOST_AUTO_TEST_CASE(test_range_make_tuple_from)
{
    ...
    bool b = true;
    {
        ...
        static_assert(
            std::is_same<decltype(copy), tuple<int, float>>::value, "");
    ...
    }
    {
        ...
        auto copy = make_tuple_from(st);
        static_assert(
            std::is_same<decltype(copy), tuple<int, float, bool>>::value, "");
    ...
    // From a list that is cut off at a point known at compile time.
    {
        ...
        BOOST_CHECK_THROW(
            make_tuple_from(take(l, rime::size_t<3>())), range::size_mismatch);
        ...
        l.push_back(6);
        {
            ...
            static_assert(
                std::is_same<decltype(copy), tuple<int, int, int>>::value, "");
...
BOOST_AUTO_TEST_CASE(test_range_tie_from)
{
    ...
    }
    {
        ...
        static_assert(
            std::is_same<
                decltype(copy), tuple<int &, float &, bool const &>>::value,
            "");
    ...
    // From a list that is cut off at a point known at compile time.
    {
        ...
        // The list can turn out to be too short at run time.
        BOOST_CHECK_THROW(
            tie_from(take(l, rime::size_t<3>())), range::size_mismatch);
...
BOOST_AUTO_TEST_CASE(test_range_forward_as_tuple)
{
    ...
    }
    {
        ...
        BOOST_MPL_ASSERT(
            (std::is_same<
                decltype(t), range::tuple<int &, float const &, double &>>) );
        BOOST_CHECK_EQUAL(first(t), 7);
        BOOST_CHECK_EQUAL(at_c<1>(t), 8.5f);
        BOOST_CHECK_EQUAL(at_c<2>(t), 12.75);
    ...
    }
    {
        ...
        double d = 13.75;
        auto t =
            range::forward_as_tuple(std::move(i), std::move(f), std::move(d));
        BOOST_MPL_ASSERT(
            (std::is_same<
                decltype(t), range::tuple<int &&, float &&, double &&>>) );
...
BOOST_AUTO_TEST_CASE(test_range_copy_tuple_from)
{
    ...
    }
    {
        ...
        static_assert(
            std::is_same<
                decltype(copy), tuple<int &, float const &, bool &>>::value,
            "");
    ...
    // From a list that is cut off at a point known at compile time.
    {
        ...
        // The list can turn out to be too short at run time.
        BOOST_CHECK_THROW(
            copy_tuple_from(take(l, rime::size_t<3>())), range::size_mismatch);
        ...
        {
            auto copy =
                copy_tuple_from(unique_view(take(l, rime::size_t<3>())));
...
BOOST_AUTO_TEST_CASE(test_range_tuple_swap)
{
...

    {
        ...
        tuple<tracked<int>, char, tracked<float>> t2(
            tracked<int>(c, 10), 'b', tracked<float>(c, 12.5f));

        BOOST_CHECK_EQUAL(
            c.counts(), utility::tracked_counts(4, 0, 4, 0, 0, 0, 0, 4));

        swap(t1, t2);
        BOOST_CHECK_EQUAL(
            c.counts(), utility::tracked_counts(4, 0, 4, 0, 0, 2, 0, 4));
