...
struct count
{
    ...
    template <class Element> void operator()(Element const &) { ++number; }
};
...
{
    BOOST_MPL_ASSERT(
        (range::has<range::callable::for_each(tuple<int>, plus)>) );
    ...
    BOOST_MPL_ASSERT_NOT((range::has<range::callable::for_each(tuple<int>)>) );

    BOOST_MPL_ASSERT_NOT(
        (range::has<range::callable::for_each(tuple<int>, float, plus)>) );
    BOOST_MPL_ASSERT_NOT(
        (range::has<range::callable::for_each(tuple<int>, int, plus)>) );
}
...
BOOST_AUTO_TEST_CASE(unrolled)
{
    ...
    // Tuple with 8 elements.
    {
        ...
        BOOST_CHECK_EQUAL(boost::any_cast<int>(c.elements[3]), -5);
        BOOST_CHECK_EQUAL(boost::any_cast<unsigned>(c.elements[4]), 900u);
        BOOST_CHECK_EQUAL(boost::any_cast<unsigned short>(c.elements[5]), 30u);
    ...
    {
        tuple<
            double, short, bool, int, unsigned, unsigned short, float, char,
            unsigned char>
        ...
        BOOST_CHECK_EQUAL(boost::any_cast<float>(c.elements[6]), 4.5f);
        BOOST_CHECK_EQUAL(boost::any_cast<char>(c.elements[7]), 'A');
        BOOST_CHECK_EQUAL(boost::any_cast<unsigned char>(c.elements[8]), 'q');
    }
}
