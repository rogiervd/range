...
BOOST_AUTO_TEST_CASE(test_range_view_shared)
{
    ...
    static_assert(
        !has<callable::size(decltype(v))>::value,
...
BOOST_AUTO_TEST_CASE(test_range_view_shared_reverse)
{
    ...
    }
    {
        ...
        BOOST_CHECK(
            empty(drop(drop(drop(drop(v, back), back), back), back), back));
    }
}
...
{
    template <class Range> auto operator()(Range const & range) const
...
auto return_2_a_hello()
    RETURNS(range::view_shared(std::make_tuple(2, 'a', std::string("hello"))));
