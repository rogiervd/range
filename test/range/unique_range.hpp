...
*/
template <class Range> inline auto unique_view(Range && range) RETURNS(
    unique_range<
        typename range::decayed_result_of<range::callable::view(Range)>::type,
        false>(range::view(std::forward<Range>(range))));
...
\internal
*/
template <class Range> inline auto one_time_view(Range && range) RETURNS(
    unique_range<
        typename range::decayed_result_of<range::callable::view(Range)>::type,
...
template <bool OneTime, class Range> inline auto implement_default_direction(
    unique_range_tag<OneTime> const &, Range && range)
    RETURNS(
        range::default_direction(range::helper::get_underlying<Range>(range)));

template <bool OneTime, class Range, class Direction> auto implement_empty(
    unique_range_tag<OneTime> const &, Range && range,
    Direction const & direction)
    RETURNS(
        range::empty(range::helper::get_underlying<Range>(range), direction));

template <bool OneTime, class Range, class Direction> auto implement_size(
    unique_range_tag<OneTime> const &, Range && range,
    Direction const & direction)
    RETURNS(
        range::size(range::helper::get_underlying<Range>(range), direction));

// first: only if OneTime = false.
// Otherwise this is implementented automatically through "chop".
template <class Range, class Direction> auto implement_first(
    unique_range_tag<false> const &, Range && range,
    Direction const & direction)
    RETURNS(
        range::first(range::helper::get_underlying<Range>(range), direction));

// drop() only takes an rvalue range, pilfers it, and deactivates the original
// range.
template <
    bool OneTime, class Underlying, class Increment, class Direction,
    class Result = unique_range<
        typename std::decay<decltype(range::drop(
            std::declval<Underlying>(), std::declval<Increment>(),
            std::declval<Direction>()))>::type,
        OneTime>>
Result implement_drop(
...
template <
    bool OneTime, class Underlying, class Direction,
